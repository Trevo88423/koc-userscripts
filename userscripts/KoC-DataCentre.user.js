...
(async function() {
  ...
  const SCRIPT_VERSION = '2.21.0'; // Must match @version above
   ...
   *
   * AAT is the ceiling on a sab run: you cannot destroy weapons they do not
   * have. 5,000 Nunchaku is 5B of destroyable value; 80 is noise.
   *
  ...
  function parseWarListPage() {
    ...
    for (const tr of [...table.querySelectorAll('tr')].slice(1)) {
      ...
      const sentryText = (tds[col.sentry] || {}).innerText || '';
      const aatText = ((tds[col.aat] || {}).innerText || '').replace(/,/g, '');
      ...
      out.push({
        ...
        sentry: firstNum(sentryText),
        sentryAgeMs: ageMs(sentryText),
        weapon: ((tds[col.weapon] || {}).innerText || '').trim() || null,
        aat: firstNum(aatText),
        sabGold: parseInt((aatText.match(/(\d+)\s*damage per sab/i) || [])[1], 10) || null
      });
    }
    return out;
  }
  ...
  async function collectFromWarList() {
    ...
    debugLog(`⚔️ War list collected ${rows.length} rows (${sentryCount} sentry, ${aatCount} AAT)`);
  }
